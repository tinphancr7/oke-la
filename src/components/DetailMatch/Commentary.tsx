...
const Commentary = ({
  ...
  const caculteHomeGoal = useMemo(() => {
    let result = 0;
    Object.keys(matchAnalysis?.homeShootTime?.total || {}).forEach(
      (item: any) => {
        result += Number(matchAnalysis?.homeShootTime?.total[item]);
      }
    );
    return result;
  }, [matchAnalysis]);
  const caculateAwayGoal = useMemo(() => {
    let result = 0;
    Object.keys(matchAnalysis?.awayShootTime?.total || {}).forEach(
      (item: any) => {
        result += Number(matchAnalysis?.awayShootTime?.total[item]);
      }
    );
    return result;
  }, [matchAnalysis]);
