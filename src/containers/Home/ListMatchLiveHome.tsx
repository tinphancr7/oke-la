import {getPlayingMatchGroupLeague} from "@/apis/match";
...
const ListMatchLiveHome = ({
	...
	const {
		...
		queryKey: ["listMatchesLive", moment(date).format("YYYY-MM-DD"), search],
		queryFn: async ({pageParam}) => {
			const res = await getPlayingMatchGroupLeague(
				pageParam,
				5,
	...
	const listMatchesLive = data?.pages?.reduce(
	...
	return (
		<div>
			<div className="mt-4">
				{listMatchesLive?.length > 0 &&
					listMatchesLive?.map((item) => (
						<>
...
export default ListMatchLiveHome;
