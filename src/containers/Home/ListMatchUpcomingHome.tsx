import {getUpcomingMatchesGroupLeague} from "@/apis/match";
...
const ListMatchUpcomingHome = ({
	...
	const {
		...
		queryKey: ["listMatchesUpcoming", moment(date).format("YYYY-MM-DD")],
		queryFn: async ({pageParam}) => {
			const res = await getUpcomingMatchesGroupLeague(
				pageParam,
				5,
	...
	const listMatchesUpcoming = data?.pages?.reduce(
	...
	return (
		<div>
			<div className="mt-4">
				{listMatchesUpcoming?.length > 0 &&
					listMatchesUpcoming?.map((item) => (
						<>
...
export default ListMatchUpcomingHome;
